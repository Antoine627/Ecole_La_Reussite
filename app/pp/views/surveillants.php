...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_surveillant'])) {
    $id_surveillant = intval($_POST['id_surveillant']);
    $nom = htmlspecialchars($_POST['nom']);
    $prenom = htmlspecialchars($_POST['prenom']);

    // Mettez à jour le surveillant dans la base de données
    $sql = "UPDATE surveillants SET nom = :nom, prenom = :prenom WHERE id_surveillant = :id_surveillant";
    $stmt = $db->prepare($sql);
    $stmt->execute([
        'nom' => $nom,
        'prenom' => $prenom,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['archive_id'])) {
    ...
    try {
        ...
        $queryUpdate = "UPDATE surveillants SET archive = 1 WHERE id_surveillant = ?";
...
$surveillants = $db->query("SELECT * FROM surveillants WHERE archive = 0")->fetchAll(PDO::FETCH_ASSOC);



if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST['id_surveillant'];
    $nom = $_POST['nom'];
    $prenom = $_POST['prenom'];
    ...
    $sql = "UPDATE surveillants SET nom=?, prenom=?, date_naissance=?, adresse=?, sexe=?, telephone=?, email=?, niveau=?, date_embauche=?, statut=? WHERE id=?";
    $stmt = $conn->prepare($sql);

    if ($stmt) {
        $stmt->bind_param("ssssssssssi", $nom, $prenom, $date_naissance, $adresse, $sexe, $telephone, $email, $niveau, $date_embauche, $statut, $id);
        if ($stmt->execute()) {
...
<head>
    ...
    <style>
      ...
      .container {
        ...
        th:nth-child(12), td:nth-child(12) { width: 50px; }
...
<div class="modal fade" id="editSurveillantModal" tabindex="-1" role="dialog" aria-labelledby="editSurveillantModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" name="id" id="editSurveillantId">
                    <div class="form-group">
                        <label for="editNom">Nom</label>
                        <input type="text" class="form-control" name="nom" id="editNom" value="<?php echo htmlspecialchars($surveillant['nom']); ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="editPrenom">Prénom</label>
                        <input type="text" class="form-control" name="prenom" id="editPrenom" value="<?php echo htmlspecialchars($surveillant['prenom']); ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="editDateNaissance">Date de Naissance</label>
                        <input type="date" class="form-control" name="date_naissance" id="editDateNaissance" max="2000-12-31" value="<?php echo htmlspecialchars($surveillant['date_naissance']); ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="editAdresse">Adresse</label>
                        <input type="text" class="form-control" name="adresse" id="editAdresse" value="<?php echo htmlspecialchars($surveillant['adresse']); ?>">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="telephone" id="editTelephone" value="<?php echo htmlspecialchars($surveillant['telephone']); ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="editEmail">Email</label>
                        <input type="email" class="form-control" name="email" id="editEmail" value="<?php echo htmlspecialchars($surveillant['email']); ?>" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" name="date_embauche" id="editDateEmbauche" value="<?php echo htmlspecialchars($surveillant['date_embauche']); ?>" required>
                    </div>
                    <div class="form-group">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Fermer</button>
                    <!-- Exemple de bouton dans votre tableau -->
                      <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#editSurveillantModal" onclick="setSurveillantData(<?php echo $surveillant['id']; ?>)">Modifier</button>
                    ...
                    <?php foreach ($surveillants as $surveillant): ?>
                        <tr>
                          <td><?php echo htmlspecialchars($surveillant['matricule']); ?></td>
                            ...
                            <td>
                                <button class="btn btn-warning" data-toggle="modal" data-target="#editSurveillantModal" data-id="<?php echo htmlspecialchars($surveillant['id_surveillant']); ?>">Modifier</button>
